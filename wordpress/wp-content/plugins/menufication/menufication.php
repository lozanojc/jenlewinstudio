...
class Menufication {
    ...
    */
    ...
    function is_enabled() {
        ...
        if( !$this->is_off_canvas_active() && !$options['enableMultiple'] && ( !$options['responsive_menu'] || strlen($options['responsive_menu']) === 0 ) && !$options['page_menu_support'] && !$options['customMenuElement'] ) {
            return false;
        }
    ...
    */
    function add_js () {
        ...
        $options = get_option( $this->plugin_prefix . '_options' );
        $myFile = $this->plugin_url . "js/jquery.menufication.min.js";

        // Add base script
        wp_register_script('menufication-js', $myFile, array('jquery') );
        ...
        // Add settings script
        wp_register_script('menufication-js-setup', $this->plugin_url . "js/menufication-setup.js", array('menufication-js', 'jquery') );
    ...
    */
    function add_admin_js () {
        ...
        if( function_exists('wp_enqueue_media') )  {
            wp_enqueue_media();
            wp_register_script('menufication-js-admin', $this->plugin_url . "js/menufication-admin.js", array('jquery') );
        } else {
            wp_enqueue_script('media-upload');
            wp_enqueue_script('thickbox');
            wp_register_script('menufication-js-admin', $this->plugin_url . "js/menufication-admin.js", array('jquery','media-upload','thickbox') );
        }
    ...
    */
    function add_stylesheets () {
        ...
        $myFile = $this->plugin_url . "css/menufication.min.css";
    ...
    */
    ...
    function menufication_nav_class( $items, $args ) {
        ...
        if($args->theme_location == $options['responsive_menu'] ) {
            $items = "<div id='". $this->plugin_prefix ."'>" . $items . "</div>";
        }
    ...
    function menufication_page_menu_class( $items, $args ) {
        ...
        if($options['page_menu_support'] === 'on' && !$options['responsive_menu'] ) {
            $items = $this->add_search_field($items);
            return "<div id='". $this->plugin_prefix ."'>" . $items . "</div>";
        }
    }

    function add_search_field($items) {
        $options = get_option( $this->plugin_prefix . '_options' );
        if($options['addSearchField'] === 'on') {
            $items = '<li class="menufication-search-holder">'. get_search_form(false) . '</li>' . $items;
        }
        return $items;
    }
    ...
    function settings_page() { ?>
        <div class="wrap">
            ...
            <p><strong>For more information, read me the <a href="<?php echo $this->plugin_url . '/readme.md'; ?>">readme.md</a> file.</strong></p>
            <p><b>This plugin works best in combination with <a href="http://codecanyon.net/user/iveo/portfolio" target="_blank">Menufication Extra Content</a></b>, a plugin which automatically generates an Off-Canvas Content Area where you can place anything from images to shortcodes...</p>
    ...
    function menu_extra_advanced_fields() {
        ...
        echo "<div id='advanced_settings'>";
            ...
            <strong>Tip:</strong> You can bind to the custom jquery events <i>menufication-done</i>, <i>menufication-reset</i> and <i>menufication-reapply</i>.<br />
    ...
    function add_settings_link($links, $file) {
        ...
        $this_plugin = plugin_basename(dirname(__FILE__) . '/menufication.php');
...
$menufication = Menufication::getInstance();
